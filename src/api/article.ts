...
export async function getArticleDetail(articleId: string): Promise<ArticleApiType> {
  const result = await axios
    .post(`${API_URL}/article/detail`, { articleId, clientType: 'vue' })
    .then((res) => {
      return res;
    })
    .catch((error) => {
...
export async function createArticle(
  ...
  const variables = { userId, title, content, clientType: 'vue' };
  const result = await axios
...
export async function updateArticle(
  ...
  const variables = { articleId, userId, title, content, clientType: 'vue' };
  const result = await axios
